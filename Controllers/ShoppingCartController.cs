            ...
            if (ModelState.IsValid)
            {
                ...
                if (cart != null)
                {
                    ...
                    foreach (var sp in cart.Items)
                    {
                        ...
                        strSanPham += "<td>" + Shop.Common.Common.FormatNumber(sp.TotalPrice, 0) + "</td>";
                        strSanPham += "</tr>";
                    ...
                    contentCustomer = contentCustomer.Replace("{{ThanhTien}}", Shop.Common.Common.FormatNumber(thanhtien, 0));
                    contentCustomer = contentCustomer.Replace("{{TongTien}}", Shop.Common.Common.FormatNumber(TongTien, 0));
                    Shop.Common.Common.SendMail("ShopOnline", "Đơn hàng #" + order.Code, contentCustomer.ToString(), req.Email);
                    ...
                    contentAdmin = contentAdmin.Replace("{{ThanhTien}}", Shop.Common.Common.FormatNumber(thanhtien, 0));
                    contentAdmin = contentAdmin.Replace("{{TongTien}}", Shop.Common.Common.FormatNumber(TongTien, 0));
                    Shop.Common.Common.SendMail("ShopOnline", "Đơn hàng mới #" + order.Code, contentAdmin.ToString(), ConfigurationManager.AppSettings["EmailAdmin"]);
                    cart.ClearCart();
